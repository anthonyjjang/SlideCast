...
from src.core.video_composer import create_slide_video, concat_videos

@celery_app.task(bind=True)
def process_presentation(self, job_id: str, pptx_path: str, output_dir: str):
    """
    ...
    try:
        ...
        for i, slide in enumerate(slides_info):
            ...
            asyncio.run(generate_tts(notes, "ko_female", audio_path))
            
            # 단일 클립 영상 생성
            clip_path = os.path.join(output_dir, f"clip_{i+1:03d}.mp4")
            create_slide_video(image_paths[i], audio_path, clip_path)
        ...
        return {"status": "success", "output_video": final_video_path}
